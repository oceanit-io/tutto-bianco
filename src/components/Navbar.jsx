import React, { useState, useEffect } from 'react'
import AOS from 'aos'

const Navbar = () => {
  const [isScrolled, setIsScrolled] = useState(false)
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 20)
    }
    window.addEventListener('scroll', handleScroll)
    return () => window.removeEventListener('scroll', handleScroll)
  }, [])

  // Garantir que as animações da navbar funcionem desde o início
  useEffect(() => {
    // Pequeno delay para garantir que o AOS já foi inicializado
    const timer = setTimeout(() => {
      AOS.refresh()
    }, 100)
    return () => clearTimeout(timer)
  }, [])

  // Atualizar AOS quando o menu mobile abrir
  useEffect(() => {
    if (isMenuOpen) {
      // Pequeno delay para garantir que o menu foi renderizado
      const timer = setTimeout(() => {
        AOS.refresh()
      }, 50)
      return () => clearTimeout(timer)
    }
  }, [isMenuOpen])

  // Fechar menu quando clicar em um link
  const handleLinkClick = () => {
    setIsMenuOpen(false)
  }

  // Prevenir scroll do body quando menu estiver aberto
  useEffect(() => {
    if (isMenuOpen) {
      document.body.style.overflow = 'hidden'
    } else {
      document.body.style.overflow = 'unset'
    }
    return () => {
      document.body.style.overflow = 'unset'
    }
  }, [isMenuOpen])

  const whatsappLink = "https://wa.me/558332680277?text=Olá!%20Gostaria%20de%20agendar%20uma%20coleta."

  return (
    <>
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-white shadow-md' : 'bg-transparent'
      }`} style={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        <div className="container mx-auto px-4 py-4" style={{ maxWidth: '100%' }}>
          <div className="flex items-center justify-between" style={{ maxWidth: '100%' }}>
            <a 
              href="#inicio" 
              className="flex items-center space-x-2" 
              data-aos="fade-right" 
              data-aos-duration="600"
              data-aos-anchor-placement="top-bottom"
            >
              <img 
                src="/logo-h-orange-transparent.png" 
                alt="Tutto Bianco" 
                className="h-16 w-auto"
                onError={(e) => {
                  e.target.style.display = 'none'
                }}
              />
            </a>
            
            <div className="hidden md:flex items-center space-x-6">
              <a 
                href="#servicos" 
                className="text-gray-700 hover:text-primary-600 transition"
                data-aos="fade-left"
                data-aos-delay="100"
                data-aos-duration="600"
                data-aos-anchor-placement="top-bottom"
              >
                Serviços
              </a>
              <a 
                href="#como-funciona" 
                className="text-gray-700 hover:text-primary-600 transition"
                data-aos="fade-left"
                data-aos-delay="200"
                data-aos-duration="600"
                data-aos-anchor-placement="top-bottom"
              >
                Como Funciona
              </a>
              <a 
                href="#unidades" 
                className="text-gray-700 hover:text-primary-600 transition"
                data-aos="fade-left"
                data-aos-delay="300"
                data-aos-duration="600"
                data-aos-anchor-placement="top-bottom"
              >
                Unidades
              </a>
              <a 
                href="#faq" 
                className="text-gray-700 hover:text-primary-600 transition"
                data-aos="fade-left"
                data-aos-delay="400"
                data-aos-duration="600"
                data-aos-anchor-placement="top-bottom"
              >
                FAQ
              </a>
              <a 
                href={whatsappLink}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-primary text-sm flex items-center gap-2"
                data-aos="fade-left"
                data-aos-delay="500"
                data-aos-duration="600"
                data-aos-anchor-placement="top-bottom"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                Pedir Coleta
              </a>
            </div>

            <button 
              className="md:hidden text-gray-700 z-[60] relative"
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              aria-label="Toggle menu"
            >
              {isMenuOpen ? (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              ) : (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                </svg>
              )}
            </button>
          </div>
        </div>
      </nav>

      {/* Menu Mobile */}
      <div 
        className={`fixed inset-0 bg-black/50 z-[55] md:hidden transition-opacity duration-300 ${
          isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
        onClick={() => setIsMenuOpen(false)}
      >
        <div 
          className={`fixed top-0 right-0 h-full bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-[60] ${
            isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
          style={{ width: 'min(320px, 85vw)', maxWidth: '100vw' }}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Botão de fechar */}
          <button
            onClick={() => setIsMenuOpen(false)}
            className="absolute top-4 right-4 text-gray-700 hover:text-gray-900 transition-colors p-2"
            aria-label="Fechar menu"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          
          <div className="flex flex-col h-full pt-24 px-6">
            <div className="flex flex-col space-y-6">
              <a 
                href="#inicio" 
                className="text-gray-700 hover:text-primary-600 transition text-lg font-medium"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="50"
                data-aos-duration="500"
              >
                Início
              </a>
              <a 
                href="#servicos" 
                className="text-gray-700 hover:text-primary-600 transition text-lg font-medium"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="100"
                data-aos-duration="500"
              >
                Serviços
              </a>
              <a 
                href="#como-funciona" 
                className="text-gray-700 hover:text-primary-600 transition text-lg font-medium"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="150"
                data-aos-duration="500"
              >
                Como Funciona
              </a>
              <a 
                href="#unidades" 
                className="text-gray-700 hover:text-primary-600 transition text-lg font-medium"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="200"
                data-aos-duration="500"
              >
                Unidades
              </a>
              <a 
                href="#faq" 
                className="text-gray-700 hover:text-primary-600 transition text-lg font-medium"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="250"
                data-aos-duration="500"
              >
                FAQ
              </a>
              <a 
                href={whatsappLink}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-primary text-sm flex items-center justify-center gap-2 mt-4"
                onClick={handleLinkClick}
                data-aos="fade-left"
                data-aos-delay="300"
                data-aos-duration="500"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                Pedir Coleta
              </a>
            </div>
          </div>
        </div>
      </div>
    </>
  )
}

export default Navbar

